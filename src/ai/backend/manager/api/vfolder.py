...
from ai.backend.common import \
    validators as \
    tx
from ai.backend.common.logging import \
    BraceStyleAdapter
from aiohttp import \
    web
...
from .auth import (admin_required,
                   auth_required,
                   superadmin_required)
from .exceptions import (BackendAgentError,
                         GenericForbidden,
                         GenericNotFound,
                         GroupNotFound,
                         InternalServerError,
...
from .manager import (ALL_ALLOWED,
                      READ_ALLOWED,
    ...
    t.Dict(
        {
            ...
            tx.AliasedKey(["unmanaged_path", "unmanagedPath"], default=None): t.String
            | t.Null,
...
async def create(request: web.Request, params: Any) -> web.Response:
    ...
    if not verify_vfolder_name(params["name"]):
        raise InvalidAPIParameters(
            f'{params["name"]} is reserved for internal operations.'
        )
    if params["name"].startswith(".") and params["name"] != ".local":
        if params["group"] is not None:
            raise InvalidAPIParameters(
                "dot-prefixed vfolders cannot be a group folder."
            )
    ...
    async with root_ctx.db.begin() as conn:
        ...
        if not unmanaged_path:
            ...
            if folder_host not in allowed_hosts:
                raise InvalidAPIParameters(
                    "You are not allowed to use this vfolder host."
                )
        ...
        if group_id is not None:
            if "group" not in allowed_vfolder_types:
                raise InvalidAPIParameters(
                    "group vfolder cannot be created in this host"
                )
            if not request["is_admin"]:
            ...
            if "user" not in allowed_vfolder_types:
                raise InvalidAPIParameters(
                    "user vfolder cannot be created in this host"
                )
        try:
        ...
        if unmanaged_path:
            insert_values.update({"host": "", "unmanaged_path": unmanaged_path})
    ...
    t.Dict(
        {
            ...
            tx.AliasedKey(["group_id", "groupId"], default=None): tx.UUID
            | t.String
            | t.Null,
        }
    )
...
async def list_folders(request: web.Request, params: Any) -> web.Response:
    ...
    async with root_ctx.db.begin() as conn:
        ...
        if request["is_superadmin"] and params["all"]:
            ...
            j = vfolders.join(
                users, vfolders.c.user == users.c.uuid, isouter=True
            ).join(groups, vfolders.c.group == groups.c.id, isouter=True)
            query = sa.select(
                [vfolders, users.c.email, groups.c.name], use_labels=True
            ).select_from(j)
            result = await conn.execute(query)
            entries = []
            for row in result:
                ...
                entries.append(
                    {
                        ...
                        "group": str(row.vfolders_group)
                        if row.vfolders_group
                        else None,
...
@superadmin_required
@server_status_required(ALL_ALLOWED)
@check_api_params(t.Dict({t.Key("id"): t.String}))
...
async def list_hosts(request: web.Request) -> web.Response:
    ...
    if default_host not in allowed_hosts:
        default_host = None
    resp = {"default": default_host, "allowed": sorted(allowed_hosts)}
    return web.json_response(resp, status=200)
...
async def list_all_hosts(request: web.Request) -> web.Response:
    ...
    if default_host not in all_hosts:
        default_host = None
    resp = {"default": default_host, "allowed": sorted(all_hosts)}
    return web.json_response(resp, status=200)
...
async def get_info(request: web.Request, row: VFolderRow) -> web.Response:
    ...
    try:
        async with root_ctx.storage_manager.request(
            ...
            "GET",
            "folder/usage",
            json={"volume": volume_name, "vfid": str(row["id"])},
            raise_for_status=True,
        ) as (_, storage_resp):
...
@server_status_required(READ_ALLOWED)
@check_api_params(t.Dict({t.Key("folder_host"): t.String}))
async def get_qtree_config(request: web.Request, params: Any) -> web.Response:
    root_ctx: RootContext = request.app["_root.context"]
    proxy_name, volume_name = root_ctx.storage_manager.split_host(params["folder_host"])
    log.info("VFOLDER.GET_QTREE_CONFIG (volume_name:{})", volume_name)
    try:
        async with root_ctx.storage_manager.request(
            proxy_name,
            "GET",
...
@check_api_params(t.Dict({t.Key("new_name"): tx.Slug(allow_dot=True)}))
async def rename_vfolder(
    ...
    allowed_vfolder_types = await root_ctx.shared_config.get_vfolder_types()
    log.info(
        "VFOLDER.RENAME (ak:{}, vf.old:{}, vf.new:{})", access_key, old_name, new_name
    )
    async with root_ctx.db.begin() as conn:
        entries = await query_accessible_vfolders(
            conn,
...
async def mkdir(request: web.Request, params: Any, row: VFolderRow) -> web.Response:
    ...
    access_key = request["keypair"]["access_key"]
    log.info(
        "VFOLDER.MKDIR (ak:{}, vf:{}, path:{})", access_key, folder_name, params["path"]
    )
    proxy_name, volume_name = root_ctx.storage_manager.split_host(row["host"])
    try:
        async with root_ctx.storage_manager.request(
...
@server_status_required(READ_ALLOWED)
@vfolder_permission_required(VFolderPermission.READ_WRITE)
@check_api_params(t.Dict({t.Key("path"): t.String, t.Key("size"): t.ToInt}))
...
@check_api_params(
    t.Dict(
        {t.Key("files"): t.List[t.String], t.Key("recursive", default=False): t.ToBool}
    )
)
...
@server_status_required(READ_ALLOWED)
@vfolder_permission_required(VFolderPermission.READ_ONLY)
@check_api_params(t.Dict({t.Key("path", default=""): t.String(allow_blank=True)}))
...
@server_status_required(ALL_ALLOWED)
@check_api_params(
    t.Dict({tx.AliasedKey(["perm", "permission"]): VFolderPermissionValidator})
)
...
async def delete_invitation(request: web.Request, params: Any) -> web.Response:
    ...
    try:
        async with root_ctx.db.begin() as conn:
            query = (
                sa.select(
                    [vfolder_invitations.c.inviter, vfolder_invitations.c.invitee]
                )
                .select_from(vfolder_invitations)
                .where(
                    (vfolder_invitations.c.id == inv_id)
...
async def share(request: web.Request, params: Any) -> web.Response:
    ...
    async with root_ctx.db.begin() as conn:
        ...
        if len(user_info) < len(params["emails"]):
            users_not_in_vfolder_group = list(
                set(params["emails"]) - set(emails_to_share)
            )
...
@admin_required
@server_status_required(ALL_ALLOWED)
@check_api_params(t.Dict({t.Key("emails"): t.List(t.String)}))
...
async def delete(request: web.Request) -> web.Response:
    ...
    try:
        async with root_ctx.storage_manager.request(
            ...
            "POST",
            "folder/delete",
            json={"volume": volume_name, "vfid": str(folder_id)},
            raise_for_status=True,
        ):
            pass
    except aiohttp.ClientResponseError:
...
async def leave(request: web.Request, row: VFolderRow) -> web.Response:
    ...
    if row["is_owner"]:
        raise InvalidAPIParameters(
            "Cannot leave a vfolder owned by the requesting user."
        )
...
async def clone(request: web.Request, params: Any, row: VFolderRow) -> web.Response:
    ...
    if not target_folder_host:
        target_folder_host = await root_ctx.shared_config.etcd.get(
    ...
    if not verify_vfolder_name(params["target_name"]):
        raise InvalidAPIParameters(
            f'{params["target_name"]} is reserved for internal operations.'
        )
        ...
        if params["target_name"].startswith("."):
            ...
            for dotfile in dotfiles:
                if params["target_name"] == dotfile["path"]:
                    raise InvalidAPIParameters(
                        "vFolder name conflicts with your dotfile."
                    )

        if "user" not in allowed_vfolder_types:
            raise InvalidAPIParameters("user vfolder cannot be created in this host")

        source_proxy_name, source_volume_name = root_ctx.storage_manager.split_host(
            source_folder_host
        )
        target_proxy_name, target_volume_name = root_ctx.storage_manager.split_host(
            target_folder_host
        )
...
@auth_required
@server_status_required(READ_ALLOWED)
@check_api_params(t.Dict({tx.AliasedKey(["vfolder_id", "vfolderId"]): tx.UUID}))
        ...
        shared_info.append(
            {
                ...
                "shared_to": {"uuid": str(shared.user), "email": shared.email},
...
async def list_mounts(request: web.Request) -> web.Response:
    ...
    resp: MutableMapping[str, Any] = {
        "manager": {"success": True, "mounts": all_mounts, "message": "(legacy)"},
        "storage-proxy": {
            "success": True,
    ...
    async def _fetch_mounts(
        sess: aiohttp.ClientSession, agent_id: str
    ) -> Tuple[str, Mapping]:
    ...
    async def _mount(sess: aiohttp.ClientSession, agent_id: str) -> Tuple[str, Mapping]:
        ...
        try:
            ...
            async with sess.post(url, json=params, headers=headers) as resp:
                if resp.status == 200:
                    data = {"success": True, "message": await resp.text()}
                else:
            ...
            log.error(log_fmt + ": timeout from watcher (ag:{})", *log_args, agent_id)
            raise
        except Exception:
            log.exception(
...
async def umount_host(request: web.Request, params: Any) -> web.Response:
    ...
    async def _umount(
        ...
        try:
            ...
            log.error(
                log_fmt + ": timeout from watcher (agent:{})", *log_args, agent_id
            )
            raise
        except Exception:
            log.exception(
